...
def blrObjFunction(initialWeights, *args):
    ...
    initialWeights = np.reshape(initialWeights, (-1, 1))
    ##################
    # YOUR CODE HERE #
    ##################
    # HINT: Do not forget to add the bias term to your input data
    train_data = np.hstack([np.ones([train_data.shape[0], 1]), train_data])
    z = np.matmul(train_data, initialWeights)
    theta = sigmoid(z)
    eq2 = np.dot((1- labeli).T,(np.log(1-theta)))
    eq1 = np.dot(labeli.T,np.log(theta))
    error = -(eq1 + eq2)/n_data
...
def blrPredict(W, data):
    ...
    z = np.matmul(data, W)
    theta = sigmoid(z)
    label = np.reshape(np.argmax(theta.T, axis=0), (-1,1))
